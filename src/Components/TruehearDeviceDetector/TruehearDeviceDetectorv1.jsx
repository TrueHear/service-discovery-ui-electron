...
const TruehearDeviceDetectorv1 = () => {
    ...
    const automatic_search = async () => {
        try {
            ...
            inSearchAutomaticMessagesRef.current = "Searching for Available interfaces..";
            setIsSearchingAutomatic(inSearchAutomaticMessagesRef.current);
            console.log(inSearchAutomaticMessagesRef.current);

            const { status, message: interfaces } = await window.electronapi.getInterfaces();
            ...
            for (let i = 0; i < interfaces.length; i++) {
                ...
                setIsSearchingAutomatic(`Searching service on interface ${i + 1}: ${iface.name} IP:${iface.address}`);
                console.log(`Searching service on interface ${i + 1}: ${iface.name}`);

                // Perform the service search for the current interface
                const { status: serviceStatus, message: serviceMessage } = await window.electronapi.searchServices({
                    mdns_address: mdnsAddress,
                    mdns_port: Number(mdnsPort),
                    interface: iface?.address || "169.254.137.22",
                    service_query: serviceQuery,
                ...
                setIsSearchingAutomatic(`Interface ${i + 1} (${iface.name}): ${isSuccess ? "✅" : "❌"}`);
                console.log(`Interface ${i + 1} (${iface.name}): ${isSuccess ? "✅" : "❌"}`);
            ...
            setMessage({ type: 'info', message: found_device ? "Services found" : `No service found on any intefaces try increasing the timeout` });
